...
def save_user(user_dir, name):
    if not os.path.exists(user_dir):
        os.makedirs(user_dir)

    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    src_image_path = 'static/now_face.jpg'
    dst_image_path = os.path.join(user_dir, f"{name}_{current_time}.jpg")
...
#             cv2.putText(frame, f'{name} ({emotion})', (left, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
#     cnt += 1
#     # 화면에 표시
